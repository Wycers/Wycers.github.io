<template>
  <v-menu bottom transition="scale-transition" :origin="origin" v-show="!!socials">
    <template slot="activator">
      <slot></slot>
    </template>
    <v-list>
      <v-list-tile ripple color="secondary" v-for="item in socials" :key="item.text" :href="item.href" target="_brank" rel="noopener noreferrer">
        <v-list-tile-avatar>
          <i class="fab fa-lg" :class="`fa-${item.icon}`"></i>
        </v-list-tile-avatar>
        <v-list-tile-title class="capitalize">{{$tt(item.text)}}</v-list-tile-title>
      </v-list-tile>
      <v-list-tile ripple color="secondary" key="copy" @click="copyLink">
        <v-list-tile-avatar>
    ...
    <input type="text" :value="url" ref="copyEl" tabindex="-1" aria-hidden="true" class="fake-hide">
  </v-menu>
</template>
<script>
