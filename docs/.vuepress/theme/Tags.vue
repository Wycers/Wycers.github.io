<template>
  <v-container grid-list-xl align-center class="blog-container py-5">
    <v-layout row wrap>
      <Tag v-for="item in $blog.tagList" :key="item" :slug="item">#{{item}} {{$blog.tags[item].length}}</Tag>
    </v-layout>
    <v-list row wrap v-if="pageList" tag="ul" class="reset elevation-1 mt-4">
      <template v-for="(slug, index) in pageList">
        <v-list-tile xs12 ripple :key="slug" :to="$blog.posts[slug].path">
        ...
        <v-divider v-if="index + 1 < pageList.length" :key="index"></v-divider>
      </template>
    </v-list>
  </v-container>
</template>
